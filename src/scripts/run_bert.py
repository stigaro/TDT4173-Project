...
from Source.Utility.visualization import save_per_class_roc_curves, save_per_class_metrics
...
checkpoint_path = PATH_TO_MODEL_BERT_CHECKPOINTS + "/cp-{epoch:04d}.ckpt"
...
model.save_pretrained(PATH_TO_MODEL_BERT_SAVED)

# Save the results
predictions = model.predict(x=test_x.values())[0]
results = ResultsExtractor(predictions)
save_per_class_metrics(results.retrieve_per_class_metrics(), PATH_TO_RESULT_BERT)
save_per_class_roc_curves(results.retrieve_per_class_roc(), PATH_TO_RESULT_BERT)
